...
func TestAcademicRateLimiter_ConcurrentAccess(t *testing.T) {
	...
	// Test concurrent access to the same source
	...
	for i := 1; i < len(requestTimes); i++ {
		...
		assert.GreaterOrEqual(t, gap, 3*time.Second, "Requests should be at least 3 seconds apart")
	}
}
